...
import model.{Agent, TerrainMap}
import org.mtrupkin.core.{Point, Size}
 ...
 */
class AStar(map: TerrainMap, size: Size) {
...
//  val size = tileMap.size
  private val nodes = ofDim[Node](size.width, size.height)
  private def nodes(p: Point): Node = nodes(p.x)(p.y)
  size.foreach(p => nodes(p.x)(p.y) = new Node(p))

  // nodes that have been searched through
  private val closed = mutable.ListBuffer.empty[Node]

  // nodes that we do not yet consider fully searched
  private var open = new mutable.PriorityQueue[Node]()

  // list of points on path from agent position to point p inclusive
  def search(agent: Agent, p: Point, maxSearchDistance: Int): Seq[Point] = {
    ...
    def removeFromOpen(node: Node) {
      node.open = false
      open = open.filterNot(_ == node)
    }

    // add a node to the closed list
    def addToClosed(node: Node) {
      node.closed = true
      closed += node
    }

    // remove a node from the closed list
    def removeFromClosed(node: Node) {
      node.closed = false
      closed -= node
    }
  ...
  protected def heuristic(agent: Agent, p0: Point, p: Point): Double = {
    val d = p - p0
