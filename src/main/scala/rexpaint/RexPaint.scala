...
import org.mtrupkin.console.{RGB, ScreenChar}
import org.mtrupkin.core.{Matrix, Size}
...
object RexPaintLayer {
  ...
  def read(is: RexPaintStream): Matrix[ScreenChar] = {
    val size = Size(is.readInt(), is.readInt())
    val m = new Matrix[ScreenChar](size)
    for {
      i <- 0 until size.width
      j <- 0 until size.height
    } m((i, j)) = readScreenChar(is)

    m
  }
}

case class RexPaintImage(name: String, size: Size, layers: Seq[Matrix[ScreenChar]])
